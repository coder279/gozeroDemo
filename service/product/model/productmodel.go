...
func (m *defaultProductModel) FindAll(page, size int64, name string) (resp []*Product, err error) {
	query := fmt.Sprintf("select * from %s", m.table)
	if name != "" {
		query = query + "where name = ? order by id desc limit ?,?"
		err = m.QueryRowsNoCache(&resp, query, name, page, size)
	} else {
